...
namespace HeronPipeline {
  internal sealed class ExportDynamoDBTable: Construct {
    ...
    public ExportDynamoDBTable(Construct scope, string id, Infrastructure infrastructure): base(scope, id)
    {
      this.scope = scope;
      this.id = id;
    ...
    public void CreateStartExportFunction()
    {
      var startTableExportFunction = new PythonFunction(this, this.id + "_startTableExportFunction", new PythonFunctionProps{
          ...
          Environment = new Dictionary<string, string> {
              {"HERON_TABLE",infrastructure.mutationsTable.TableArn},
